...
func TestMinimumCardPickupHashMap(t *testing.T) {
	type args struct {
		cards []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := MinimumCardPickupHashMap(tt.args.cards); got != tt.want {
				t.Errorf("MinimumCardPickupHashMap() = %v, want %v", got, tt.want)
			}
		})
	}
}
